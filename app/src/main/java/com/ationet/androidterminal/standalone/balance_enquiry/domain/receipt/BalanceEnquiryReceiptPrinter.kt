package com.ationet.androidterminal.standalone.balance_enquiry.domain.receipt
...
class BalanceEnquiryReceiptPrinter(
...
//                printSignature()
    ...
    private fun printTransactionInfo(
        ...
        if (configuration.printInvoiceNumber) {
            ContextCompat.getContextForLanguage(context).getString(R.string.receipt_invoice_number).splitWordLimit(21).forEach { line ->
                printer.write(
                    text = line,
                    format = TextFormat(
                        alignment = Alignment.Left,
            ...
            invoice.splitWordLimit(21).forEach { line ->
                printer.write(
                    text = line,
                    format = TextFormat(
                        alignment = Alignment.Left,
    ...
    private fun printErrorTransactionInfo(
        ...
        if (configuration.printInvoiceNumber && !invoice.isNullOrBlank()) {
            ...
            authorizationCode.splitWordLimit(21).forEach { line ->
                printer.write(
                    text = line,
                    format = TextFormat(
                        alignment = Alignment.Left,
    ...
    private fun printProducts(
        ...
        printer.write(
            text = ContextCompat.getContextForLanguage(context).getString(
                ...
                LocaleFormatter.formatNumber(product.amount.toString(), 2, context)
            ),
            format = TextFormat(
                alignment = Alignment.Left,
                linefeed = true
        ...
        printer.write(
            text = ContextCompat.getContextForLanguage(context).getString(
                R.string.receipt_quantity,
                LocaleFormatter.formatNumber(product.quantity.toString(), 3, context)
            ),
            format = TextFormat(
                alignment = Alignment.Left,
                linefeed = true
        ...
        /*if (configuration.printTransactionDetails) {
            ...
            val companyPrice = CompanyPrice(product.modifiers, product.quantity)
            ...
            if (companySurcharges > 0) {
                printer.write(
                    text = ContextCompat.getContextForLanguage(context)
                        .getString(R.string.receipt_surcharges, LocaleFormatter.formatNumber(companySurcharges.toString(), 2, context)),
                    format = TextFormat(
                        alignment = Alignment.Right,
                        linefeed = true
                    )
            ...
            if (product.amount != null && (companyDiscounts > 0 || companySurcharges > 0)) {
                printer.feedPaper()
                printer.write(
                    text = ContextCompat.getContextForLanguage(context).getString(R.string.total, LocaleFormatter.formatNumber(product.amount.toString(), 2, context)),
                    format = TextFormat(
                        alignment = Alignment.Right,
                        linefeed = true,
