...
fun OperationTypeScreen(
    viewModel: OperationTypeViewModel = hiltViewModel(),
    onFuelSelected: () -> Unit,
    onSkuSelected: () -> Unit,
    onBack: () -> Unit,
    onExit: () -> Unit
    ...
    when (viewModel.state.collectAsStateWithLifecycle().value) {
        OperationTypeState.ListOperationType -> {
            OptionList(
                viewModel = viewModel,
...
@Composable
fun OptionList(
    viewModel: OperationTypeViewModel,
    onFuelSelected: () -> Unit,
    onSkuSelected: () -> Unit,
    onBack: () -> Unit,
    onExit: () -> Unit
    ...
    Surface(modifier = modifier) {
        Box(modifier = Modifier.padding(horizontal = 16.dp)) {
            Column(
                ...
                AATHeaderTitle(stringResource(R.string.option))
