package com.ationet.androidterminal.standalone.preauthorization.domain.receipt
...
import com.ationet.androidterminal.core.domain.hal.printer.PrinterStatus as HALPrinterStatus

class PreAuthorizationReceiptPrinter @Inject constructor(
    ...
    override suspend fun printReceipt(receipt: Receipt): PrinterStatus {
        ...
        return when (result) {
            HALPrinterStatus.Ok -> PrinterStatus.Ok
            HALPrinterStatus.OutOfPaper -> PrinterStatus.OutOfPaper
            HALPrinterStatus.Busy -> PrinterStatus.Error(
                errorCode = PrinterErrorCodes.BUSY
            )

            HALPrinterStatus.Overheat -> PrinterStatus.Error(
                errorCode = PrinterErrorCodes.OVER_HEAT
            )

            HALPrinterStatus.UnderVoltage -> PrinterStatus.Error(
                errorCode = PrinterErrorCodes.UNDER_VOLTAGE
            )

            HALPrinterStatus.DriverError -> PrinterStatus.Error(
                errorCode = PrinterErrorCodes.DRIVER_ERROR
            )

            HALPrinterStatus.Error -> PrinterStatus.Error(
    ...
    private fun printProducts(
        ...
        if (pumpId != null) {
            printer.write(
                text = ContextCompat.getContextForLanguage(context)
                    .getString(R.string.receipt_pump, pumpId.toString()),
                format = TextFormat(
                    alignment = Alignment.Left,
                    linefeed = true
                )
    ...
    private fun printProductInRow(
        ...
        val formattedQuantity = if (quantity == null) {
            ""
        } else {
            LocaleFormatter.formatNumber(quantity?.toString(), 3, context)
        }
